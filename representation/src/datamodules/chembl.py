...
SPLIT_OPTIONS = ["full", "unique"]
RANDOM_TYPE_OPTIONS = ["unrestricted", "restricted"]


@AIDD.DataRegistry.register_arguments(key="tdc_chembl")
@dataclass
class TDCommonsArgs:
    name: str = "tdc_chembl"
...
@AIDD.DataRegistry.register(key="tdc_chembl")
class TDCommonsDatamodule(pl.LightningDataModule):
    def __init__(self, **kwargs) -> None:
        ...
        self.usecols: list[str] = [self.smiles_col]
        self.mapping: dict[str, Union[str, list[str]]] = {}
    ...
    def prepare_data(self):
        ...
        if self.use_raw:
            ...
            self.train = self.load_data(f"{self.root}/data/chembl_{self.split}_train.csv")
            self.valid = self.load_data(f"{self.root}/data/chembl_{self.split}_valid.csv")
            self.test = self.load_data(f"{self.root}/data/chembl_{self.split}_test.csv")
            self.predict = None
