...
class ConstantScalingUnit(pl.LightningModule):
    ...
    def forward(self, x: Tensor) -> Tensor:
        y = 0.5 * x + 0.5 * self.relu1(self.conv1(x)) * self.relu2(self.conv2(x))
        return y
...
class DropoutShortcutUnit(pl.LightningModule):
    ...
    def forward(self, x: Tensor) -> Tensor:
        y = self.dropout(x) + self.relu1(self.conv1(x)) * self.relu2(self.conv2(x))
        return y
...
class ConvolutionShortcutUnit(pl.LightningModule):
    ...
    def forward(self, x: Tensor) -> Tensor:
        y = self.relu3(self.conv3(x)) + self.relu1(self.conv1(x)) * self.relu2(self.conv2(x))
        return y
