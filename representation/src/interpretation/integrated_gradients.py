...
class ExplainabilityIntegratedGradients(Callback):
    def __init__(self, include_baseline: bool = True, save_dir: str = "integrated_gradients") -> None:
        super().__init__()
        self.include_baseline = include_baseline
        self.save_dir = save_dir

    def on_predict_start(self, trainer: pl.Trainer, pl_module: pl.LightningModule) -> None:
        self.ig = IntegratedGradients(pl_module.predict_proba_embed)  # type: ignore
        pd.DataFrame().to_csv(validate_or_create_full_path(f"{self.save_dir}/ig.csv"), header=True, index=False)
        pd.DataFrame(columns=["ig_delta"]).to_csv(
            validate_or_create_full_path(f"{self.save_dir}/ig_delta.csv"), header=True, index=False
        )
    ...
    def on_predict_batch_end(
        ...
        pd.DataFrame(attributions).to_csv(f"{self.save_dir}/ig.csv", mode="a", header=False, index=False)
        pd.DataFrame(pd.Series(delta)).to_csv(f"{self.save_dir}/ig_delta.csv", mode="a", header=False, index=False)
